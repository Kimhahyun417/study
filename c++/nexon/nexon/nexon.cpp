...
public:
    ...
    bool isValid(int r1, int c1, int r2, int c2)
    {
        ...
        cerr << "[DEBUG] Checking rectangle: (" << r1 << "," << c1 << ") ~ (" << r2 << "," << c2 << ")\n";

        for (int r = r1; r <= r2; r++)
            for (int c = c1; c <= c2; c++)
            {
                cerr << "   board[" << r << "][" << c << "] = " << board[r][c] << "\n";
                if (board[r][c] != 0)
                {
                    sums += board[r][c];
                    if (r == r1) r1fit = true;
        ...
        cerr << "   → Sum = " << sums << ", borders = " << (r1fit && r2fit && c1fit && c2fit ? "OK" : "Missing") << "\n";
    ...
    {
        printBoard(); // 디버깅용 보드 출력

        int maxArea = 0;
        vector<int> bestMove = { -1, -1, -1, -1 };
        vector<int> manu = bestMove;
        ...
        for (int r1 = 0; r1 < numRows; r1++)
        {
            for (int c1 = 0; c1 < numCols; c1++)
            {
                ...
                {
                    for (int c2 = c1; c2 < numCols; c2++)
                    {
                        // 최소 2칸 이상을 사용하는 사각형만 탐색
                        if ((r2 > r1 || c2 > c1) && isValid(r1, c1, r2, c2))
                        {
...
int amain()
{
    ...
    {
        string line;
        getline(cin, line);  // READY FIRST SECOND 
